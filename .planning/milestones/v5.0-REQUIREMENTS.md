# Requirements Archive: v5.0 Horizontal Worker Architecture
...
- [x] **EXEC-01**: Worker process selects execution strategy via WORKER_ARCH env variable (vertical | horizontal)
- [x] **EXEC-02**: Generic JobExecutor interface defines execute(jobName, jobData) contract that all strategies implement
- [x] **EXEC-03**: VerticalExecutor runs job handler in-process within the pg-boss worker (current behavior, zero regression)
- [x] **EXEC-04**: HorizontalExecutor launches an ECS RunTask per job, passing job name and serialized job data via container overrides
...
- [x] **INFRA-01**: ECS task definition for ephemeral worker containers using the same Docker image with a different command
- [x] **INFRA-02**: IAM task execution role and task role with permissions for STS AssumeRole, RDS, S3, Bedrock access
- [x] **INFRA-03**: Pulumi code provisions task definition, security group, and CloudWatch log group for ephemeral workers
...
|-------------|-------|--------|
| EXEC-01 | Phase 22 | Complete |
| EXEC-02 | Phase 22 | Complete |
| EXEC-03 | Phase 22 | Complete |
