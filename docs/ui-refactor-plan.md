...
- **Phase 5** (data-layer rollout) — IN PROGRESS:
    ...
    - ⏭ NEXT — remaining raw-fetch pages: settings/members, inventory data layer (page.tsx fetchResourcesRef/fetchSyncStatus), dashboard sections (refreshKey→invalidation, multi-file), knowledge-base, per-channel settings forms. Add `lib/queries/<domain>.ts` per domain. Mirror the established pattern.
