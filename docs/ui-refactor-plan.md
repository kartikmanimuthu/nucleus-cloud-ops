...
- **Phase 6** (visual overhaul, D1=full) — IN PROGRESS:
    ...
    - **6c-ext** ⏭ NEXT — wire EmptyState into remaining zero-states (accounts-table, schedules-table/list) AND add a shared `<PageHeader>` (title + description + actions slot) — every page repeats the sticky header block (`flex items-center justify-between sticky top-0 ... h1 + p + buttons`). Extract it, wire accounts + schedules pages.
