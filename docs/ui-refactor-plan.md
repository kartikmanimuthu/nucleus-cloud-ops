...
- **Phase 5** (data-layer rollout) — IN PROGRESS:
    ...
    - ⏭ NEXT — remaining raw-fetch pages: inventory data layer, dashboard sections (refreshKey→invalidation, multi-file), knowledge-base, per-channel settings forms, org/provider/mcp settings forms. Add `lib/queries/<domain>.ts` per domain. Mirror the established loader-gate pattern for edit forms.
