...
- **Phase 5** (data-layer rollout) — IN PROGRESS:
    ...
    - ⏭ NEXT — remaining raw-fetch pages: inventory data layer (page.tsx fetchResourcesRef/fetchSyncStatus), dashboard sections (refreshKey→invalidation, multi-file), knowledge-base, settings/members/roles, per-channel settings forms. Add `lib/queries/<domain>.ts` per domain (inline fetch if no client service). Mirror the established pattern.
