...
- **Phase 6** (visual overhaul, D1=full) — IN PROGRESS:
    ...
    - **6d** ⏭ NEXT — zustand store for UI state. Highest-value remaining (user explicitly requested zustand). Replace the localStorage-in-Context in `components/theme-config-provider.tsx` (theme config: theme name, radius, font, mode) with a zustand store using `persist` middleware. Keep the ThemeConfigContext API/consumers working (or provide a `useThemeConfig` hook with the same shape). Verify SSR-safety (zustand persist + Next: guard hydration).
