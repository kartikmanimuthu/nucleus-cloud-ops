...
- **Phase 6** (visual overhaul, D1=full) — IN PROGRESS:
    ...
    - **6c-ext3** ⏭ NEXT — remaining headers with `text-3xl font-bold tracking-tight`: `components/dashboard/dashboard-header.tsx`, `app/app/settings/page.tsx`, `app/app/settings/roles/page.tsx`, `app/app/schedules/settings/page.tsx`, `app/app/inventory/settings/page.tsx`, right-sizing page, knowledge-base pages. Each has slightly different markup — inspect per file. Also wire EmptyState into table zero-states (accounts-table line ~377, schedules-table/list) — note these are inside TableCell colSpan, so wrap appropriately.
    - **6d** — zustand store for UI/theme-config state (replace localStorage-in-Context in theme-config-provider).
