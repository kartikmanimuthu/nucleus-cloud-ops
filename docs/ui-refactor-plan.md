...
- **Phase 6** (visual overhaul, D1=full) — IN PROGRESS:
    ...
    - **6c** ⏭ NEXT — reusable `<EmptyState>` primitive (icon + title + description + optional action) and wire it into list pages' zero-states (accounts/schedules/inventory tables when empty). Also consider a shared page-header pattern. Safe/objective; commit the primitive + a couple of wirings.
