...
- **Phase 5** (data-layer rollout) — IN PROGRESS:
    ...
    - ⏭ NEXT — remaining raw-fetch pages: inventory data layer (page.tsx still uses fetchResourcesRef/fetchSyncStatus raw), channels settings pages, dashboard, knowledge-base, settings/members/roles. Add `lib/queries/<domain>.ts` per domain (inline fetch if no client service). Mirror the established pattern.
