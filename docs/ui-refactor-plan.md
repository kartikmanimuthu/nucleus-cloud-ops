...
- **Phase 3** ⏭ NEXT (forms) — migrate the ~7 manual `useState` dialogs to RHF + zod (reuse `components/ui/form.tsx`). Targets: `components/accounts/create-account-dialog.tsx`, `edit-account-dialog.tsx`, `components/schedules/create-schedule-dialog.tsx` (+ check edit/duplicate/delete dialogs). Mutations should use the query hooks (useCreateAccount/useUpdateSchedule/etc.) so cache invalidation is automatic. Pattern reference: existing `app/app/accounts/create/create-account-form.tsx` (already RHF+zod).
