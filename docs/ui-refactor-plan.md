...
- **Phase 4** ⏭ NEXT (Zod 3→4) — DEDICATED breaking upgrade. Steps: bump `zod@^4` (+ check `@hookform/resolvers` peer supports z4; may need bump) + `@t3-oss/env-nextjs` may need `^0.13`. Then sweep ~18 zod files for v4 API changes (notably: `z.string().email()/.url()` deprecated→`z.email()/z.url()`; error map/message API; `.optional()` infer unchanged). Run FULL `bunx tsc --noEmit` (not just touched files) since this is global. Agent tools.ts has the densest schemas — but those feed AI logic; only adjust zod API calls, do NOT change agent behavior.
