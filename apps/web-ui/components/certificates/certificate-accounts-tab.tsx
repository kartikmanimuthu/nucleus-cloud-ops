...
import { useCertificateAccounts, useReimportCertificate } from "@/lib/queries/certificates";
...
export function CertificateAccountsTab({ certificateId }: CertificateAccountsTabProps) {
    const router = useRouter();
    const { data: accounts = [], isLoading } = useCertificateAccounts(certificateId);
    ...
    if (accounts.length === 0) {
        return (
            <div className="p-8 text-center text-muted-foreground">
                No linked accounts yet. Click{" "}
                <span className="font-medium">Discover / Rescan</span> to scan ACM across your
                active accounts, or <span className="font-medium">Deploy to Account</span> to
                            ...
                            <TableRow key={account.accountId}>
                                ...
                                <TableCell>
                                    <Badge variant="outline">
                                        {account.resourceCount} resource(s)
                                    </Badge>
                                </TableCell>
                                ...
                                <TableCell className="text-right">
                                    <div className="flex items-center justify-end gap-2">
                                        ...
                                        <Button
                                            ...
                                            size="sm"
                                            className="gap-1 h-8"
                                            disabled={reimportingId === account.accountId}
                                            onClick={() => handleReimport(account.accountId)}
                                        >
                                            {reimportingId === account.accountId ? (
