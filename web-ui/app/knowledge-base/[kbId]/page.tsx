...
export default function KnowledgeBaseDetailPage() {
  ...
  const fetchKB = useCallback(async () => {
    try {
      ...
      if (!res.ok) {
        if (res.status === 404) { toast.error('Knowledge base not found'); router.push('/knowledge-base'); return; }
  ...
  return (
    <div className="flex-1 p-4 md:p-8 pt-6 bg-background">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        {/* Back nav */}
        <button onClick={() => router.push('/knowledge-base')} className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors">
        ...
        <div className="flex items-center justify-between">
          ...
          <Button variant="default" size="sm" className="gap-1.5" onClick={() => router.push(`/knowledge-base/ask?kb=${kbId}`)}>
        ...
        <Card>
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              ...
              <Button variant="outline" size="sm" className="gap-1.5" onClick={() => router.push(`/knowledge-base/${kbId}/sources/new`)}>
